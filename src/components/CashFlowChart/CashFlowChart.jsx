...
export default function CashFlowChart() {
  ...
  const [lineKey, setLineKey]     = useState(0)   // bump to retrigger draw animation
  const svgRef                    = useRef(null)

  useEffect(() => {
    setLoading(true)
    api.forecastPoints(days)
      .then(d => { setData(d); setLoading(false); setLineKey(k => k + 1) })
      ...
      <div className={styles.chartWrap}>
        ...
        {!loading && chart && (
          ...
          >
            ...
            {/* Main line — animated draw-in using stroke-dasharray */}
            <path
              key={lineKey}
              d={chart.path}
              fill="none"
